...
def save_images(pipeline, step, samples_dir, num_images=4, num_inference_steps=25):
    ...
    grid = torchvision.utils.make_grid(
        [transforms.ToTensor()(img) for img in images], nrow=2
    )
    ...
    if resume_from:
        ...
        if last_checkpoint == "checkpoint-final":
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid checkpoint directory format: {last_checkpoint}. Expected 'checkpoint-N' or 'checkpoint-NNNN'"
                ) from e
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--resume_from",
        type=str,
        help="Path to checkpoint directory to resume from (e.g., output/train_2024.../checkpoints/checkpoint-005)",
    )
